...
namespace libff
{
    ...
    FieldT SHA512_rng(const uint64_t idx)
    {
        assert(
            GMP_NUMB_BITS ==
            64); // current Python code cannot handle larger values, so testing here for some assumptions.
        ...
        do
        {
            ...
            EVP_DigestUpdate(mdctx, &idx, sizeof(idx));
            EVP_DigestUpdate(mdctx, &iter, sizeof(iter));
